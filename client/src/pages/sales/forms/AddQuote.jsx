...
//               <button 
//                 onClick={id ? editQuote :saveQuote}
//                 disabled={isSaving}
...
//                 {id ? 'Modifier ce devis...' : 'Ajouter le Devis'}
//               </button>
//             </div>
//           </div>
//         </div>
...
function AddQuote({ onClose }) {
  ...
  const removeItem = (itemId) => {
    if (quoteData.items.length === 1) {
      alert("Vous ne pouvez pas supprimer le dernier article");
      return;
    }
    setQuoteData(prev => ({
      ...prev,
  ...
  // UPDATE ITEM LINE WITH CALCULATIONS
  const updateItemLine = (itemId, field, value) => {
    setQuoteData(prev => ({
      ...prev,
      items: prev.items.map(item => {
        if (item.id === itemId) {
      ...
      <div className="p-6">
                  ...
                  <tr key={item.id} className="border-b">
                    ...
                    <td className="p-3">
                      <input
                        type="number"
                        value={item.quantity}
                        onChange={(e) => updateItemLine(item.id, 'quantity', parseInt(e.target.value) || 1)}
                        className="w-20 border rounded px-2 py-1"
                        min="1"
                      />
                    </td>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          <div className="flex gap-4 justify-between">
            ...
            >
              <FaSave />
              {isSaving ? 'En cours...' : id ? 'Modifier le Devis' : 'Ajouter le Devis'}
            </button>
          </div>
        </div>
      </div>
